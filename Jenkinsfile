pipeline {
    ...
    stages {
        ...
        stage('build and Tag docker image') {
            steps {
                script {
                        sh "docker build -t vikasshinde206/ekart:latest -f docker/Dockerfile ."
                    }
            }
        }
        ...
        stage('Push image to Hub'){
            steps{
                script{
                   ...
                   sh 'docker login -u vikasshinde206 -p ${dockerhubpwd}'}
                   sh 'docker push vikasshinde206/ekart:latest'
                }
            }
        }
